...
class Level implements ChunkManager, Metadatable{
		...
		if($optimizeRebuilds){
			...
			foreach($blocks as $b){
				...
				if($b instanceof Block){
					$pk->blockRuntimeId = ($b->getId() << 4) | ($b->getDamage() & 0xf);
				}else{
					$fullBlock = $this->getFullBlock($b->x, $b->y, $b->z);
					$pk->blockRuntimeId = $fullBlock;
				}
	...
	public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, bool $createParticles = false) : bool{
		...
		if($player !== null){
			...
			if(!$ev->isCancelled() && !$this->server->getProperty("player.anti-cheat.allow-instant-break", false)) {
				...
				$actualTime = ceil($player->getStartingBreakTime(true));
				$expectedTime -= 1;
	...
	private function destroyBlockInternal(Block $target, Item $item, ?Player $player = null, bool $createParticles = false) : void{
		if($createParticles && $this->server->getProperty('definitions.particles.enabled', true) && $this->server->getProperty('definitions.particles.destroy-block', true)) {
	...
	public function useItemOn(Vector3 $vector, Item &$item, int $face, Vector3 $clickVector = null, Player $player = null, bool $playSound = false) : bool{
		...
		if($playSound && $this->server->getProperty('definitions.sounds.enabled', true) && $this->server->getProperty('definitions.sounds.place-block', true)) {
			$this->broadcastLevelSoundEvent($hand, LevelSoundEventPacket::SOUND_PLACE, ($hand->getId() << 4) | ($hand->getDamage() & 0xf));
		}
		...
		if(isset($this->chunkSendQueue[$index = ((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF)])){
			foreach($this->chunkSendQueue[$index] as $player){
				...
				if($player->isConnected() and isset($player->usedChunks[$index])){
					$player->sendChunk($x, $z, $this->chunkCache[$index][ProtocolUtils::convertToChunk($player->getProtocol())]);
				}
			}
